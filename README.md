...
`pgxgen` consists of 3 main packages:

- `github.com/tkcrm/pgxgen/cmd/pgxgen` is a command line tool to be
  invoked in `go:generate` directives.
- `github.com/tkcrm/pgxgen/gen` is the back end for the
  `github.com/tkcrm/pgxgen/cmd/pgxgen` tool. All the command line
  flags exposed by the command line tool have corresponding fields in
  the `Config` struct exposed by this package, so pgxgen can be used
  as a library as well as a command line tool.
- `github.com/tkcrm/pgxgen` and its subpackages (besides `cmd` and `gen`)
  contain common types and utilities that need to be shared between client code and the
  code generated by `pgxgen`.

# Development

See [HACKING.md](HACKING.md) for information about working on the pgxgen
codebase.

# Using `pgxgen`

The `pgxgen` command line tool has a fairly simple interface. You need
to tell pgxgen how to connect to the postgres database that you want to
...
tell pgxgen where to put the code that it generates via the `-o` option,
but you can also just accept the default output name. Finally, you must
point `pgxgen` at a `toml` file which contains the bulk of the configuration
telling `pgxgen` what part of the database schema to generate code for.

`pgxgen` will not generate code for any database object unless it is explicitly
asked do so so via an entry in the config file. Often, configuring `pgxgen`
to generate code for an object is as simple as adding that object's name
to the config file and letting `pgxgen` figure out the rest, but there are
finer grained knobs if you want more control.

## Configuration

`pgxgen` is configured with a `toml` file. Some of the configuration options have already
...
An example file can be found at [`cmd/pgxgen/test/models/pgxgen.toml`](cmd/pgxgen/test/models/pgxgen.toml).
...
`pgxgen` offers two main features: automatic generation of shims wrapping
SQL queries and automatic generation of go structs from SQL tables.

### Query Shims

`pgxgen` knows how to infer the return and argument types of queries, so all
you have to write is the SQL that you want to execute using standard postgres
$N placeholder syntax for parameters and pgxgen will generate simple go
wrapper functions that perform all the boilerplate needed to call the
query from go. `pgxgen` will automatically generate a struct to contain
...
`pgxgen` will generate a return type
...
`*sql.Rows` as and argument. `pgxgen` will also generate two functions
...
If you don't provide a name for your return type `pgxgen` is happy to
make one up, but if you want to override the fairly uninspired name
that `pgxgen` will come up with, you can do so. This feature is also
...
returned via a query, so by default `pgxgen` will generate boxed fields
...
configuration option to tell `pgxgen` not to box the fields in question.
If you are re-using a return type between queries, be sure to apply this
flag consistently when dealing with columns that appear in multiple `pgxgen`
...
null flags, as `pgxgen` will automatically infer the nullness of the fields
...
`pgxgen` translates table definitions into golang structs along with
...
structs generated by `pgxgen` as return values from your own custom
...
`pgxgen` would generate the following structs for the two different tables.
...
pgxgen to generate the same sort of model structs by explicitly telling it about
...
names, while golang structs conventionally have singular names, so `pgxgen` will convert
plural table names to singular names. This is important for interop with `gorm`, as `gorm`
imposes the same rule on table vs struct names. In fact, `pgxgen` and `gorm` use exactly
...
`pgxgen` has noticed the foreign key constraint on the `attachments` table and inferred that `Attachment`
is a child entity of `SmallEntity`. Child entities are not automatically filled in by the various accessor
methods when a record is fetched from the database, but `pgxgen` does generate utility code
...
- Values (constant or variable definitions)
  - <Entity><FieldName>FieldIndex
    - For each field in the entity, `pgxgen` generates a constant indicating the field's
...
Mostly, `pgxgen` doesn't know anything about the semantics of the fields of the tables
it generates code for, it just generates type safe converters for the fields and leaves
the semantics up to higher level code. There are a few types of fields that `pgxgen` will
manipulate or rely on though.

##### Primary Keys

Every table that `pgxgen` generates a model for must have exactly one primary key field.
...
`pgxgen` will infer relationships between tables based on the foreign key constraints
...
tracking the life cycle of the object. By default, `pgxgen` won't do anything with
timestamps, but if the `created_at_field`, `updated_at_field`, or `deleted_at_field`
keys are set, either globally or on a specific table in the toml file, `pgxgen`
...
`pgxgen` can automatically detect relationships between tables via foreign key constraints
in the database. If `pgxgen` notices a foreign key from table A to table B, it will assume
that A belongs to B, and generate a member field in the generated struct for table B
containing a slice of As. If there is a `UNIQUE` index on the foreign key, `pgxgen` will
...
with your data model `pgxgen` provides configuration options to explicitly control the
...
query results. To support these use cases `pgxgen` supports registering statements in
...
`pgxgen` aims to generate models which are compatible with the `gorm` tool. We have a lot
of code which uses `gorm` already and some people may prefer using `gorm` over the
routines that `pgxgen` provides. `pgxgen` can still help those people by taking care of
the drudge work of writing model structs which match up with the database table
definitions. pgxgen's GORM compatibility is focused on covering the most common
cases that we've encountered in practice, so it may not be complete. If you encounter
a way in which pgxgen-generated structs are not compatible with GORM usage, try
...
`pgxgen` follows semver. Any breaking change will be indicated by an appropriate bump
of the version number as defined by the semver spec.

The minimum supported go language version of pgxgen is 1.11. `pgxgen` will not consider
...
`pgxgen` is not the only database first code generator for go that works with postgres out
there.

## pgxgen vs [xo](https://github.com/xo/xo)

`pgxgen` and `xo` are similar in many ways. Both are command line tools which connect directly
...
## pgxgen vs [sqlc](https://github.com/kyleconroy/sqlc)
...
any other database first code generator we are aware of, including pgxgen. The tradeoff
...
|                                                 | `pgxgen`                                     | `xo`                                                  | `sqlc`                                                | notes                                                                                                                                                                                                                                                                                                                                         |
| ----------------------------------------------- | -------------------------------------------- | ----------------------------------------------------- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Multiple RDBMS support                          | no                                           | yes                                                   | yes                                                   | We trapped ourselves with the name here a bit, but if we did want to support another RDBMS we would be able to re-use much of the `pgxgen` internals in new binaries.                                                                                                                                                                         |
| Configuration                                   | toml file                                    | command line flags                                    | magic comments in sql files                           | File based configuration like `sqlc` and `pgxgen` use can mean faster code generation.                                                                                                                                                                                                                                                        |
| Supports custom code generation templates       | no                                           | yes                                                   | no                                                    |                                                                                                                                                                                                                                                                                                                                               |
| Tries to generate idiomatic code                | no (prefers correct code)                    | yes                                                   | yes                                                   | A big difference here is the way that the different libraries scan results sets. Both `xo` and `sqlc` generate straight line “obvious” code that is easy to read, but `pgxgen` generates more complex code which is harder to understand but will not error unexpectedly when a database migration is run while the application is running.   |
| Infers relationships between tables             | yes                                          | no                                                    | no                                                    | `pgxgen` will automatically add pointer and slice fields connecting tables associated with one another via foreign keys. These tables can be filled in with `pgxgen`'s includes system.                                                                                                                                                       |
| Transaction/Connection Support                  | Yes (explicit calls on the generated client) | Yes (via a wrapper interface for the database handle) | Yes (via a wrapper interface for the database handle) | Both `xo` and `sqlc` accept interfaces containing a subset of the methods on `*sql.DB` which allows the same code to be used for connection pools, connections, and transactions. `pgxgen` provides a specific wrapper struct for each of the different database connection handle types.                                                     |
| Style of Generated Query Routine                | methods                                      | free functions                                        | methods                                               | `xo` generates free functions which accept the database connection interface as well as the parameters, while `pgxgen` and `sqlc` attach queries to a single handle struct that contains all operations                                                                                                                                       |
| Lifecycle timestamp support                     | yes                                          | no                                                    | no                                                    | `pgxgen` can be configured to automatically fill in `created_at` and `deleted_at` timestamps.                                                                                                                                                                                                                                                 |
| Soft deletion support                           | yes                                          | no                                                    | no                                                    | `pgxgen` can be configured to respect and fill in life cycle timestamps with its default CRUD methods, though custom queries still need to manually account for these soft deletion timestamps.                                                                                                                                               |
| Can be called as a library in addition to a CLI | yes                                          | no                                                    | no                                                    | tkcrm internally uses the library interface to better integrate with a bazel based build system.                                                                                                                                                                                                                                              |
| Volume of generated code                        | high                                         | medium                                                | medium                                                | If a small generated database access layer is important to you, `pgxgen` may not yet be the best choice for you                                                                                                                                                                                                                               |
| Default update allows partial updates           | yes                                          | no                                                    | n/a                                                   | `pgxgen`'s update CRUD routines allow you to configure which fields are updated with a bitset. Both `sqlc` and `xo` can use custom statements to handle granular updates, but they cannot deal with dynamically choosing which fields to update at runtime quite as easily.                                                                   |
| Default upsert support                          | yes                                          | yes                                                   | no                                                    | `pgxgen`'s upsert is more flexible but not as simple to use, while `xo`'s upsert is a little simpler to work with. You must use a custom statement for upsert with `sqlc`.                                                                                                                                                                    |
| Infers good names for query arguments           | no                                           | no                                                    | yes                                                   | `sqlc` can automatically infer names for query arguments in the generated go code by noticing which fields the arguments are compared with. This type of feature is possible due to `sqlc`'s unique approach to getting database schema metadata. With `pgxgen`, you must explicitly configure names if you want them to be better than arg0. |
| Supports RETURNING                              | no                                           | no                                                    | yes                                                   | Due to the way that `pgxgen` and `xo` get database metadata by creating temporary views, they are unable to support the RETURNING keyword in queries. Because `sqlc` parses the database schema, it can more easily support RETURNING.                                                                                                        |
| Representation of NULL values                   | pointers                                     | `Null*` types from the `"database/sql"` package       | `Null*` types from the `"database/sql"` package       | Here `pgxgen` chooses to expose nullable values as boxed values, which is less efficient than using the `Null*` types from the `"database/sql"`, but we believe is more ergonomic.                                                                                                                                                            |
| Generates code for all tables in schema         | no                                           | yes                                                   | yes                                                   | `pgxgen` only generates code for tables that you have explicitly asked it to generate code for.                                                                                                                                                                                                                                               |
