...
//
// In order to focus on just a single example, run with pgxgen_TEST_EXAMPLE=<examples>
...
import (
	...
	ensureSchema "github.com/tkcrm/pgxgen/tools/ensure-schema/lib"
)
...
func TestExamples(t *testing.T) {
	...
	// afterward
	err = ensureSchema.PopulateDB(path.Join("..", "cmd", "pgxgen", "test", "db.sql"))
	chkErr(t, err)
}
...
func getExampleAllowed() func(exampleName string) bool {
	focusedExamples, inEnv := os.LookupEnv("pgxgen_TEST_EXAMPLE")
	if !inEnv {
