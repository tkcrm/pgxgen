...
func (p *pgClientImpl) bulkInsertUser(
	...
	for _, v := range values {
		...
		if !defaultFields.Test(UserBioFieldIndex) {
			args = append(args, &nullConvertUserBio{valid: true, value: &v.Bio})
		}
		if !defaultFields.Test(UserConfigFieldIndex) {
			args = append(args, &nullConvertconfig__pgxgenMODSEP__Config{valid: true, value: &v.Config})
		}
		if !defaultFields.Test(UserHomepageFieldIndex) {
			args = append(args, v.Homepage)
		}
	}
...
const (
	...
	UserBioFieldIndex      int = 2
	UserConfigFieldIndex   int = 3
	UserHomepageFieldIndex int = 4
	UserMaxFieldIndex      int = (5 - 1)
)

// A field set saying that all fields in User should be updated.
// For use as a 'fieldMask' parameter
var UserAllFields pgxgen.FieldSet = pgxgen.NewFieldSetFilled(5)

var defaultableColsForUser = func() pgxgen.FieldSet {
	fs := pgxgen.NewFieldSet(UserMaxFieldIndex)
...
var fieldsForUser []fieldNameAndIdx = []fieldNameAndIdx{
	...
	{name: `bio`, idx: UserBioFieldIndex},
	{name: `config`, idx: UserConfigFieldIndex},
	{name: `homepage`, idx: UserHomepageFieldIndex},
}
...
func (p *pgClientImpl) updateUser(
	...
	if fieldMask.Test(UserBioFieldIndex) {
		args = append(args, &nullConvertUserBio{valid: true, value: &value.Bio})
	}
	if fieldMask.Test(UserConfigFieldIndex) {
		args = append(args, &nullConvertconfig__pgxgenMODSEP__Config{valid: true, value: &value.Config})
	}
	if fieldMask.Test(UserHomepageFieldIndex) {
		args = append(args, value.Homepage)
	}
...
func (p *pgClientImpl) bulkUpsertUser(
	...
	if hasConflictAction {
		...
		if fieldMask.Test(UserBioFieldIndex) {
			updateCols = append(updateCols, `bio`)
			updateExprs = append(updateExprs, `excluded.bio`)
		}
		if fieldMask.Test(UserConfigFieldIndex) {
			updateCols = append(updateCols, `config`)
			updateExprs = append(updateExprs, `excluded.config`)
		}
		if fieldMask.Test(UserHomepageFieldIndex) {
			updateCols = append(updateCols, `homepage`)
			updateExprs = append(updateExprs, `excluded.homepage`)
		}
		if len(updateCols) > 1 {
			stmt.WriteRune('(')
		}
...
type DBQueries interface {
	...
	// User methods
	GetUser(ctx context.Context, id int64, opts ...pgxgen.GetOpt) (*User, error)
	ListUser(ctx context.Context, ids []int64, opts ...pgxgen.ListOpt) ([]User, error)
	InsertUser(ctx context.Context, value *User, opts ...pgxgen.InsertOpt) (int64, error)
	BulkInsertUser(ctx context.Context, values []User, opts ...pgxgen.InsertOpt) ([]int64, error)
	UpdateUser(ctx context.Context, value *User, fieldMask pgxgen.FieldSet, opts ...pgxgen.UpdateOpt) (ret int64, err error)
	UpsertUser(ctx context.Context, value *User, constraintNames []string, fieldMask pgxgen.FieldSet, opts ...pgxgen.UpsertOpt) (int64, error)
	BulkUpsertUser(ctx context.Context, values []User, constraintNames []string, fieldMask pgxgen.FieldSet, opts ...pgxgen.UpsertOpt) ([]int64, error)
	DeleteUser(ctx context.Context, id int64, opts ...pgxgen.DeleteOpt) error
	BulkDeleteUser(ctx context.Context, ids []int64, opts ...pgxgen.DeleteOpt) error
	UserFillIncludes(ctx context.Context, rec *User, includes *include.Spec, opts ...pgxgen.IncludeOpt) error
	UserBulkFillIncludes(ctx context.Context, recs []*User, includes *include.Spec, opts ...pgxgen.IncludeOpt) error
...
type User struct {
	...
	Bio      UserBio       `gorm:"column:bio"`
	Config   config.Config `gorm:"column:config"`
	Homepage []byte        `gorm:"column:homepage"`
}
...
var scannerTabForUser = [...]func(*User, *nullableScanTgtsForUser) interface{}{
	...
	func(
		...
		return &nullConvertUserBio{valid: true, value: &r.Bio}
	},
	func(
		r *User,
		nullableTgts *nullableScanTgtsForUser,
	) interface{} {
		return &nullConvertconfig__pgxgenMODSEP__Config{valid: true, value: &r.Config}
	},
	func(
		r *User,
		nullableTgts *nullableScanTgtsForUser,
...
type nullConvertconfig__pgxgenMODSEP__Config struct {
	valid bool
	value *config.Config
}

func (n *nullConvertconfig__pgxgenMODSEP__Config) Scan(value interface{}) error {
	if value == nil {
		n.value, n.valid = &config.Config{}, false
		return nil
	}

	buff, isByteArray := value.([]byte)
	if !isByteArray {
		return fmt.Errorf("scanning config.Config: expecting a []byte")
	}
	if string(buff) == "null" {
	...
	if n.value == nil {
		n.value = &config.Config{}
	}
	err := json.Unmarshal(buff, n.value)
	if err != nil {
		return fmt.Errorf("scanning config.Config: %s", err.Error())
	}
	n.valid = true

	return nil
}
func (n nullConvertconfig__pgxgenMODSEP__Config) Value() (driver.Value, error) {
	if !n.valid {
		return nil, nil
	}
	...
	if err != nil {
		return nil, fmt.Errorf("marshalling config.Config: %s", err.Error())
	}

	return buff, nil
}

func convertnullConvertconfig__pgxgenMODSEP__Config(v nullConvertconfig__pgxgenMODSEP__Config) *config.Config {
	if !v.valid {
		return nil
	}
...
type nullConvertUserBio struct {
	valid bool
	value *UserBio
}

func (n *nullConvertUserBio) Scan(value interface{}) error {
	if value == nil {
		n.value, n.valid = &UserBio{}, false
		return nil
	}

	buff, isByteArray := value.([]byte)
	if !isByteArray {
		return fmt.Errorf("scanning UserBio: expecting a []byte")
	}
	if string(buff) == "null" {
		// postgres returns NULL json values as null literals, pgxgen represents them
		// with go nulls.
	...
	err := json.Unmarshal(buff, n.value)
	if err != nil {
		return fmt.Errorf("scanning UserBio: %s", err.Error())
	}
	n.valid = true

	return nil
}
func (n nullConvertUserBio) Value() (driver.Value, error) {
	if !n.valid {
		return nil, nil
	}
	...
	buff, err := json.Marshal(n.value)
	if err != nil {
		return nil, fmt.Errorf("marshalling UserBio: %s", err.Error())
	}

	return buff, nil
}

func convertnullConvertUserBio(v nullConvertUserBio) *UserBio {
	if !v.valid {
		return nil
	}
