...
if [[ "${DB_HOST+x}" == "x" ]] ; then
    ...
    createdb -h "${DB_HOST}" -W test -U postgres -w -e pgxgen_test 2>/dev/null || /bin/true
fi

go generate ./...

psql "$DB_URL" < cmd/pgxgen/test/db.sql
...
if [[ -n "${LINT+x}" ]] ; then
    ...
    go test -p 1 ./...
    DB_DRIVER=postgres go test -p 1 ./cmd/pgxgen/test # re-run using lib/pq as the driver
else
    ...
    go test -race -p 1 ./...
    DB_DRIVER=postgres go test -race -p 1 ./cmd/pgxgen/test # re-run using lib/pq as the driver
fi
