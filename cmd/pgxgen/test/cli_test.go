...
// cli_test.go contains tests that execute pgxgen as a subprocess and perform
...
[[query]]
    name = "returns_text"
		`,
		exitCode: 0,
		stdoutRE: "pgxgen: doing nothing because an enable var failed to match",
	},
	{
		name: "EmptyQueryBody",
		toml: `
...
[[table]]
	...
	},
	{
		...
		toml: `
[[type_override]]
	pkg = "github.com/tkcrm/pgxgen/examples/query/models" # note lack of quotes
		`,
		exitCode: 1,
...
func TestCLI(t *testing.T) {
	debug := false
	debugEnvVar := os.Getenv("pgxgen_DEBUG_CLI")
	if debugEnvVar == "1" || debugEnvVar == "true" {
		debug = true
	}

	testDir, err := ioutil.TempDir("", "pgxgen_cli_test")
	chkErr(t, err)
	if !debug {
	...
	exe := path.Join(testDir, "pgxgen")
	mainSrc := path.Join(repoRoot, "cmd", "pgxgen", "main.go")
...
func runCLITest(
	...
	outPath := path.Join(outModule, "pgxgen.gen.go")
