...
func TestTimestampsBoth(t *testing.T) {
	...
	mask := pgxgen.NewFieldSet(models.TimestampsBothMaxFieldIndex)
...
func TestDisableTimestamps(t *testing.T) {
	...
	id, err := txClient.InsertTimestampsBoth(ctx, &models.TimestampsBoth{
		...
		CreatedAt: &date,
		UpdatedAt: date,
	}, pgxgen.InsertDisableTimestamps)
	chkErr(t, err)
	...
	id, err = txClient.UpdateTimestampsBoth(ctx, model, mask, pgxgen.UpdateDisableTimestamps)
	chkErr(t, err)
...
func TestUpsertUpdateTimestamps(t *testing.T) {
	...
	updateMask := pgxgen.NewFieldSet(global_ts_models.TimestampsGlobalMaxFieldIndex)
	...
	dip := "dip"
	fetched.Payload = &dip
	_, err = txClient.UpsertTimestampsGlobal(ctx, fetched, nil, updateMask, pgxgen.UpsertUsePkey)
	chkErr(t, err)
...
func TestSoftDeleteCRUD(t *testing.T) {
	...
	_, err = txClient.GetSoftDeletable(ctx, id)
	if err == nil || !pgxgen.IsNotFoundError(err) {
	...
	err = txClient.DeleteSoftDeletable(ctx, id, pgxgen.DeleteDoHardDelete)
	chkErr(t, err)
