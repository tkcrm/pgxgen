...
func TestSmallEntityUpdate(t *testing.T) {
	...
	noOpBitset := pgxgen.NewFieldSet(2)
...
func TestNoInfer(t *testing.T) {
	...
	if has {
		t.Fatalf("pgxgen generated a NoInfer field when we asked it not to")
	}
}
...
func TestExplicitBelongsTo(t *testing.T) {
	...
	if !has {
		t.Fatalf("pgxgen generated failed to generate ExplicitBelongsTo")
	}

	if f.Type.Kind() == reflect.Array {
		t.Fatalf("pgxgen generated a 1-many instead of a 1-1")
	}
}
...
func TestExplicitBelongsToMany(t *testing.T) {
	...
	if !has {
		t.Fatalf("pgxgen generated failed to generate ExplicitBelongsToManies")
	}

	if f.Type.Kind() != reflect.Slice {
		t.Fatalf("pgxgen generated a 1-1 instead of 1-many")
	}
}
...
func TestUpsertDoesntUpdateThingsNotInFieldSet(t *testing.T) {
	...
	empty := pgxgen.NewFieldSet(models.SmallEntityMaxFieldIndex)
	...
	id, err = txClient.UpsertSmallEntity(ctx, &models.SmallEntity{
		Id:    id,
		Anint: 14,
	}, []string{}, empty, pgxgen.UpsertUsePkey)
	chkErr(t, err)
...
func TestUpsertWithExplicitConstraints(t *testing.T) {
	...
	otherMask := pgxgen.NewFieldSet(models.ConstraintMaxFieldIndex)
...
func TestBasicCycle(t *testing.T) {
	...
	cycle2IDMask := pgxgen.NewFieldSet(models.Cycle1MaxFieldIndex)
...
func TestCycleTree(t *testing.T) {
	...
	cycle3IDMask := pgxgen.NewFieldSet(models.CycleTreeCycle1MaxFieldIndex)
...
func TestNewColumn(t *testing.T) {
	...
	if has {
		t.Fatalf("pgxgen generated an F2 field. There is something wrong with the db state.")
	}
...
func TestCustomReferenceNames(t *testing.T) {
	...
	for _, n := range names {
		...
		if !has {
			t.Fatalf("pgxgen failed to generate '%s'", n)
		}
	}
...
func TestListSucceedOnPartial(t *testing.T) {
	...
	for _, p := range params {
		entities, err := pgClient.ListSmallEntity(ctx, p, pgxgen.ListSucceedOnPartialResults)
		if err != nil {
...
func TestDefaultInserts(t *testing.T) {
	...
	}, pgxgen.InsertDefaultFields(models.DefaultValueAllFields))
	chkErr(t, err)
	...
	partialDefaults := pgxgen.NewFieldSet(models.DefaultValueMaxFieldIndex)
	partialDefaults.Set(models.DefaultValueDefaultedStringFieldIndex, true)
	id, err = txClient.InsertDefaultValue(
		ctx, &models.DefaultValue{}, pgxgen.InsertDefaultFields(partialDefaults))
	chkErr(t, err)
	vals, err = txClient.GetDefaultValue(ctx, id)
	chkErr(t, err)
	...
	}, nil, models.DefaultValueAllFields, pgxgen.UpsertDefaultFields(models.DefaultValueAllFields))
	chkErr(t, err)
	vals, err = txClient.GetDefaultValue(ctx, id)
	chkErr(t, err)
	if vals.DefaultedInt != 42 {
	...
	_, err = txClient.UpsertDefaultValue(ctx,
		...
		pgxgen.UpsertDefaultFields(models.DefaultValueAllFields),
	)
	chkErr(t, err)
	vals, err = txClient.GetDefaultValue(ctx, id)
	chkErr(t, err)
...
// tests that pgxgen can generate reasonable shim code to convert back and forth
