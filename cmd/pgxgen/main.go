...
import (
	...
	"github.com/tkcrm/pgxgen/gen"
)

func usage(ok bool) {
	usage := `
Usage: pgxgen [<options>] <config-file>

Args:
 <config-file> A configuration toml file containing a list of database objects
               that pgxgen should generate code for.
...
-d, --disable-var <var-pattern>              If <var-pattern> matches against the environment,
                                             ...
                                             than once, in which case pgxgen is disabled if any
                                             match against the environment.

-e, --enable-var <var-pattern>               If <var-pattern> matches against the environment,
                                             run pgxgen, otherwise don't do anything. If both
                                             enable vars and disable vars are provided, pgxgen
