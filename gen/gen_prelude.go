...
func (g *Generator) genPrelude() error {
	...
	preludeName := filepath.Join(filepath.Dir(g.config.OutputFileName), "pgxgen_prelude.gen.go")
...
func genBulkInsertStmt(
	...
	pkeyName string,
	includeID bool,
	defaultFieldSet pgxgen.FieldSet,
) string {
...
type pgxgenSinkScanner struct {}
func (s *pgxgenSinkScanner) Scan(value interface{}) error {
	return nil
}
...
type pgxgenNullTime struct {
	Time time.Time
	Valid bool
}
func (n *pgxgenNullTime) Scan(value interface{}) error {
	if value == nil {
...
func (n pgxgenNullTime) Value() (driver.Value, error) {
	if !n.Valid {
		return nil, nil
	}
	return n.Time, nil
...
func convertNullTime(t pgxgenNullTime) *time.Time {
	if t.Valid {
		return &t.Time
	}
	return nil
