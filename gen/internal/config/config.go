...
type DbConfig struct {
	// The name of the field that should be updated by pgxgen's generated
...
type QueryConfig struct {
	...
	// query arguments in the genrated code (normaly pgxgen will just make up
	...
	// `pgxgen` will generate code that returns just a single result rather
	// than a slice that always has just one record you have to unpack.
	// If `single_result` is true, `pgxgen` will not generate a *Query method
...
type TableConfig struct {
	...
	Name string `toml:"name"`
	// If true, pgxgen will not infer a relationship between this table
