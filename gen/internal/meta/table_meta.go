...
import (
	...
	"github.com/tkcrm/pgxgen/gen/internal/names"
	"github.com/tkcrm/pgxgen/gen/internal/types"
	"github.com/tkcrm/pgxgen/include"
)

// tablesMeta contains information _all_ of the tables that pgxgen is awair of
...
// more specific to `pgxgen`'s internal needs and contain some computed
// fields.
...
type ColMeta struct {
	...
	// that pgxgen computes and user provided tags)
	Tags string
}
...
func (tr *tableResolver) typeInfoOfCol(conf *config.TableConfig, colName string, colType string) (*types.Info, error) {
	...
	nullConverterTypeName := strings.ReplaceAll("nullConvert"+jsonOverride.TypeName, ".", "__pgxgenMODSEP__")
...
func (n *{{ .ConverterTypeName }}) Scan(value interface{}) error {
	...
	if string(buff) == "null" {
		// postgres returns NULL json values as null literals, pgxgen represents them
		// with go nulls.
		n.value, n.valid = &{{ .TargetTypeName }}{}, false
		return nil
	}
