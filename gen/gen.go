...
import (
	...
	"github.com/tkcrm/pgxgen/gen/internal/log"
	"github.com/tkcrm/pgxgen/gen/internal/meta"
	"github.com/tkcrm/pgxgen/gen/internal/types"
	"github.com/tkcrm/pgxgen/gen/internal/utils"
)

// `pgxgen.Config` contains a collection of configuration options for the
// the codegenerator
type Config struct {
	// The path to a configuration file in TOML format containing information
	// about the database objects that pgxgen should generate code for.
	...
	// A list of var patterns which disable pgxgen when they match the environment.
	DisableVars []string
...
// An instantiation of a pgxgen codegenerator
type Generator struct {
	// The user supplied configuration for this run of the pgxgen
	// codegenerator.
	config Config
...
func (g *Generator) Gen() error {
	if g.disabledByDisableVar {
		g.log.Info("pgxgen: doing nothing because a disable var matched\n")
		return nil
	}

	if g.disabledByEnableVar {
		g.log.Info("pgxgen: doing nothing because an enable var failed to match\n")
		return nil
	}
	...
	// generate imports
	_, err = out.WriteString("// Code generated by pgxgen DO NOT EDIT.\n")
	if err != nil {
		return err
	}
...
func (g *Generator) setupGenEnv() (*config.DbConfig, error) {
	g.log.Infof("pgxgen: using config '%s'\n", g.config.ConfigFilePath)
