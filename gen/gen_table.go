...
func (g *Generator) genTables(into io.Writer, tables []config.TableConfig) error {
	...
	g.imports[`"github.com/tkcrm/pgxgen/include"`] = true
	g.imports[`"github.com/tkcrm/pgxgen/unstable"`] = true
	g.imports[`"github.com/tkcrm/pgxgen"`] = true
...
var {{ .GoName }}AllFields pgxgen.FieldSet = pgxgen.NewFieldSetFilled({{ len .Meta.Info.Cols }})

var defaultableColsFor{{ .GoName }} = func() pgxgen.FieldSet {
	fs := pgxgen.NewFieldSet({{ .GoName }}MaxFieldIndex)
	{{- range .Meta.Info.Cols }}
	{{- if .DefaultExpr }}
...
func (p *PGClient) Update{{ .GoName }}(
	ctx context.Context,
	value *{{ .GoName }},
	fieldMask pgxgen.FieldSet,
	opts ...pgxgen.UpdateOpt,
) (ret {{ .PkeyCol.TypeInfo.Name }}, err error) {
...
func (p *pgClientImpl) private{{ $.GoName }}Fill{{ .GoPointsFromFieldName }}(
	...
	if !inMap {
		return fmt.Errorf("internal pgxgen error: table not pre-loaded")
	}
...
func (p *pgClientImpl) private{{ $.GoName }}FillParent{{ .GoPointsToFieldName }}(
	...
	childLoadedTab, inMap := loadedRecordTab[` + "`" + `{{ .PointsFrom.Info.PgName }}` + "`" + `]
	if !inMap {
		return p.client.errorConverter(fmt.Errorf("internal pgxgen error: table not pre-loaded"))
	}
	childIDToRecord := childLoadedTab.(map[{{ .PointsFrom.Info.PkeyCol.TypeInfo.Name }}]*{{ .PointsFrom.Info.GoName }})
