...
func (s *crud) process(args []string) (map[string][]byte, error) {
	...
	if connString == "" {
		return nil, errors.New("undefined PostgreSQL connection link. use flag -c")
	}
	...
	result := make(map[string][]byte, len(tableKeys))
...
func (s *crud) saveFile(data []byte, tableName, path string) error {
	...
	if err := utils.CraetePath(path); err != nil {
		return errors.Wrap(err, "create path error")
	}
