...
func (s *sqlc) process(args []string) error {
	...
	for _, cfg := range s.config.Pgxgen.Sqlc {
		...
		for _, modelPath := range filteredModelPaths {
			...
			if err != nil {
				return fmt.Errorf("failed to read model file dir %s: %w", modelFileDir, err)
			}
		...
		s.logger.Infof("successfully procesed sqlc models in: %s", time.Since(timeStart))
	}

	// generate go constants
	if err := s.goConstants.GenerateConstants(); err != nil {
		return fmt.Errorf("generate constatns error: %w", err)
	}
